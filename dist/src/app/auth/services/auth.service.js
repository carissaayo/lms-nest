...
let AuthService = class AuthService {
    ...
    async register(body) {
        const { email, password, confirmPassword, phoneNumber, firstName, lastName, role, } = body;
        ...
        if (existingUser) {
            throw custom_handlers_1.customError.conflict('Email has already been used ', 409);
        }
        try {
    ...
    async login(loginDto, req) {
        ...
        try {
            ...
            if (!isPasswordValid) {
                await (0, auth_utils_1.handleFailedAuthAttempt)(user, this.usersRepo);
            }
